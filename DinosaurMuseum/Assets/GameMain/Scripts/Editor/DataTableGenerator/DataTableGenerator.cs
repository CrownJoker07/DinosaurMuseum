...
namespace EasyARProject.Editor.DataTableTools
{
	public sealed class DataTableGenerator
	{
		...
		private const string TableFileSuffix = ".csv";
		...
		public static DataTableProcessor CreateDataTableProcessor(string dataTableName)
		{
			...
			System.IO.DirectoryInfo docPath = dataPath.Parent.Parent;
			return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(docPath.FullName + "/Doc/excel_output/csv", dataTableName + TableFileSuffix)), Encoding.GetEncoding("UTF-8"), 0, 1, null, 0, 2, 1);
		}
		...
		private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
		{
			...
			codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "EasyARProject");
			codeContent.Replace("__DATA_TABLE_CLASS_NAME__", "DR" + dataTableName);
			codeContent.Replace("__DATA_TABLE_COMMENT__", dataTableName + "。");
